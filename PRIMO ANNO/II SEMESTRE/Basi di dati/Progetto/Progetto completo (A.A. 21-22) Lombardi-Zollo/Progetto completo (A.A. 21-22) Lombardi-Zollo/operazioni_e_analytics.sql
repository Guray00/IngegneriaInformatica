...
CREATE PROCEDURE topologiaEdificio(IN _codEdificio CHAR(5))
	BEGIN
...
CREATE PROCEDURE rischiAnnui(IN _codEdificio CHAR(5))
	BEGIN
...
FOR EACH ROW
    ...
    SET manodopera =	(
							SELECT	O.Manodopera
							FROM	Operaio O
							WHERE	O.CFisc = NEW.CFiscLavoratore
						);
...
CREATE PROCEDURE costoMaterialiStadio(IN _codProgetto CHAR(5), IN _dataInizio DATE, OUT costoMateriali_ FLOAT)
...
BEGIN
	...
	DECLARE lavoroTarget CHAR(5);
	DECLARE codProgettoStadio CHAR(5);
	DECLARE dataInizioStadio DATE;
    
	SELECT	M.Quantita, M.CodLavoro
	INTO	quantita, lavoroTarget
	FROM	Materiale M
	WHERE	M.NomeFornitore = NEW.NomeFornitore
			AND
			M.CodLotto = NEW.CodLotto;

	SELECT	L.CodProgetto, L.DataInizioStadio
	INTO	codProgettoStadio, dataInizioStadio
	FROM	Lavoro L
	WHERE	L.CodLavoro = lavoroTarget;
...
CREATE PROCEDURE valutaAlert(IN _timestamp TIMESTAMP, IN _IDSensore CHAR(5), OUT edificio_ CHAR(5), OUT piano_ INT, OUT vano_ CHAR(5), OUT parte_ VARCHAR(100), OUT pericolosita_ FLOAT)
    ...
    BEGIN
		...
		SET esistente =	(
							SELECT	COUNT(*)
							FROM	Alert A
							WHERE	A.Timestamp = _timestamp
									AND
                                    A.IDSensore = _IDSensore
						);

		IF(esistente = 0) THEN
			SIGNAL SQLSTATE '45000'
			SET MESSAGE_TEXT = 'Alert non esistente!';
		END IF;
		...
		SET	soglia =	(
							SELECT	S.Soglia
                            FROM	Sensore S
                            WHERE	S.IDSensore = _IDSensore
                        );
		
		SET pericolosita_ =	(
								SELECT	A.ValoreMisurato
								FROM	Alert A
								WHERE	A.Timestamp = _timestamp
										AND
                                        A.IDSensore = _IDSensore
...
CREATE PROCEDURE materialiLavoro(IN _codLavoro CHAR(5))
	
    BEGIN
    
		WITH MaterialiTarget AS	(
									SELECT	M.Tipo, M.CodLotto, M.NomeFornitore, M.Quantita
                                    FROM	Materiale M
                                    WHERE	M.CodLavoro = _codLavoro
								)
		(
            ...
            FROM	MaterialiTarget M
					INNER JOIN
                    Intonaco I ON M.CodLotto = I.CodLotto AND M.NomeFornitore = I.NomeFornitore
        )
        UNION
		(
			SELECT	M.Tipo, M.CodLotto, M.NomeFornitore, M.Quantita, CONCAT	(
            ...
            FROM	MaterialiTarget M
					INNER JOIN
                    Pietra P ON M.CodLotto = P.CodLotto AND M.NomeFornitore = P.NomeFornitore  
        )
        UNION
		(
			SELECT	M.Tipo, M.CodLotto, M.NomeFornitore, M.Quantita, CONCAT	(
            ...
            FROM	MaterialiTarget M
					INNER JOIN
                    Mattone MA  ON M.CodLotto = MA.CodLotto AND M.NomeFornitore = MA.NomeFornitore
        )
        UNION
		(
			SELECT	M.Tipo, M.CodLotto, M.NomeFornitore, M.Quantita, CONCAT	(
            ...
            FROM	MaterialiTarget M
					INNER JOIN
                    Piastrella P ON M.CodLotto = P.CodLotto AND M.NomeFornitore = P.NomeFornitore
        )
		UNION
		(
			SELECT	M.Tipo, M.CodLotto, M.NomeFornitore, M.Quantita, CONCAT (
            ...
            FROM	MaterialiTarget M
					INNER JOIN
                    Altro A ON M.CodLotto = A.CodLotto AND M.NomeFornitore = A.NomeFornitore       
        );
