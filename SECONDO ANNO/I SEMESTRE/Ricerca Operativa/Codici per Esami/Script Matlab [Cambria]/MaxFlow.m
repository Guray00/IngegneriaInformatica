function [v, xT, xL, xU, p, cTnew, Tnew, cLnew, Lnew, cUnew, Unew] = MaxFlow(uT, T, uL, L, uU, U, s, t, m, passi)
% Introduce in autonomia l'arco fittizio. cL,xL si riferisce all'ordine in L, così come cT, xT con T e cU, xU con U.
% m è il numero di nodi
% s è il nodo sorgente, t quello di destinazione
% cX(:, 1) è il costo, cX(:, 2) è la portata
% uL, uT, uU sono vettori colonna con le stesse righe di L,T e U che rappresentano le portate degli archi
% Le matrici T,L,U sono matrici a due colonne dove:
%   - ogni riga indica un arco
%   - la prima colonna il nodo di partenza, la seconda quello di arrivo
% passi è il numero di iterazioni che si vuole fare (se omesso ne fa 1)


if ( isempty(T) || isempty(uT) || ...
     isempty(L) || isempty(uL) || ...
     isempty(U) || isempty(uU) || ...
     any(size(T, 1) ~= size(uT, 1)) || ...
     any(size(L, 1) ~= size(uL, 1)) || ...
     any(size(U, 1) ~= size(uU, 1)) || ...
     (size(T, 2) ~= 2) || ...
     (size(U, 2) ~= 2) || ...
     (size(L, 2) ~= 2) || ...
     (size(uT, 2) ~= 1) || ...
...
cT = [zeros(size(uT, 1), 1), uT; -1 Inf];
cU = [zeros(size(uU, 1), 1), uU];
cL = [zeros(size(uL, 1), 1), uL];
T = [T; t s];


n = [length(cT), length(cL), length(cU)];       % [T, L, U]
b = zeros(m, 1);                                % Numero nodi
v = Inf;
xT = [];
xL = [];
xU = [];
