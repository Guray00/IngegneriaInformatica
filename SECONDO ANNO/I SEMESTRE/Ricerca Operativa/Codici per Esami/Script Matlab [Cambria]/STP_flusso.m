function [v, xT, xL, p, cTnew, Tnew, cLnew, Lnew] = STP_flusso(cT, T, cL, L, b, passi)
% cL,xL si riferisce all'ordine in L, così come cT, xT con T.
...
% passi: indica il numero di iterazioni dell'algoritmo (se omesso => 1)
...
function [p, potDeg] = Potenziali(cT, T, cL, L, m)
    ...
    for i = 1 : size(L, 1)
        ...
        if(ckl < 0)
            ...
            % Prendo quello che viola di più Belman
            if(potDeg(2) == 0 || ckl < potDeg(1))
                potDeg = [ckl, i];
            end
        end
        fprintf("\n");
...
% Se miglioramento è infinito allora il problema non ha soluzione
    ...
    h = any(T == Cneg(1, 1), 2) & any(T == Cneg(1, 2), 2);
    h = find(h == 1);
