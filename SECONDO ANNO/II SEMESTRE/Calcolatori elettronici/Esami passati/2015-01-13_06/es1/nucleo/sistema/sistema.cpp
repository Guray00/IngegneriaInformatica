...
des_sock array_des_sock[MAX_SOCK];
natl next_sock;

extern "C" natl c_socket()
{
	if (next_sock >= MAX_SOCK)
		return 0xffffffff;

	natl n = next_sock++;

	des_sock *s = &array_des_sock[n];
	s->state = SOCK_AVAIL;
	s->connecting = s->accepting = 0;

	return n;
}

bool sock_valid(natl id)
{
	return id < next_sock;
}

bool sock_src_ok(des_sock *s)
{
	return s->state == SOCK_AVAIL;
}

bool sock_dest_ok(des_sock *s)
{
	return s->state == SOCK_ACCEPTING;
}

bool sock_can_accept(des_sock *s)
{
	return (s->state == SOCK_AVAIL || s->state == SOCK_ACCEPTING);
}
//   ESAME 2015-01-13 )
...
extern "C" void c_accept(natl id){
	if(!sock_valid(id)){
		flog(LOG_WARN, "id socket non valido in accept");
		c_abort_p();
		return;
	}
	...
	if(!sock_can_accept(s)){
		flog(LOG_DEBUG,"ACCEPT RESTITUISCE 0xFFF ALLA CHIAMATA DEL PROCESSO %d", esecuzione->precedenza);
...
extern "C" void c_connect(natl src, natl dest){
	if(!sock_valid(src) || !sock_valid(dest)){
		flog(LOG_WARN, "id socket non valido in accept");
		c_abort_p();
		return;
	}

	des_sock* socketSRC = &array_des_sock[src];
	des_sock* socketDEST = &array_des_sock[dest];

	if(!sock_dest_ok(socketDEST) || !sock_src_ok(socketSRC)){
	...
	natl idS3 = c_socket();
	des_sock* S3 = &array_des_sock[idS3];

	S3->state=SOCK_CONNECTED;
	socketSRC->state=SOCK_CONNECTED;

	des_proc* work = rimozione_lista(socketDEST->accepting);

	socketDEST->state = (socketDEST->accepting) ? SOCK_ACCEPTING : SOCK_AVAIL;

	work->contesto[I_RAX] = (natq)idS3;
	...
	if(s->state==SOCK_CONNECTING){
		...
		S3->state=SOCK_CONNECTED;
		s->state=SOCK_AVAIL;
		des_proc* work = rimozione_lista(s->connecting);

		work->contesto[I_RAX] = (natq)true;
		work->src->state = SOCK_CONNECTED;
		work->src = nullptr;

		esecuzione->contesto[I_RAX] = idS3;
		inspronti();
		inserimento_lista(pronti,work);
		schedulatore();
	}
