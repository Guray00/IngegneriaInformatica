...
// ( SOLUZIONE 2014-01-13 #1
	...
	des_sem* s1 = &array_dess[sem];
	s1->counter++;
	if(!s1->pointer) return;
	inspronti();
	
	des_proc* work = rimozione_lista(s1->pointer);
	
	if(!work->other_sem) 
		inserimento_lista(pronti,work);
	else{
		work->other_sem->counter--;
		if(work->other_sem->counter>=0){
			work->other_sem=nullptr;
			inserimento_lista(pronti,work);
		}
		else{
			inserimento_lista(work->other_sem->pointer,work);
			work->other_sem=s1;
		}
	}
	schedulatore();
...
// ( ESAME 2014-01-13
extern "C" void c_sem_multiwait(natl sem1, natl sem2)
{
	des_sem *s1, *s2;
