...
des_channel array_deschan[MAX_CHANNELS];
...
extern "C" natl c_channel_init(natl size){
	...
	natl indice = next_channel;
	next_channel++;
	des_channel* nuovoCanale = &(array_deschan[indice]);

	flog(LOG_WARN, "creato canale %d", indice);

	nuovoCanale->wait_r=nullptr;
	nuovoCanale->wait_w=nullptr;
	nuovoCanale->msg_buf = new natl[size];
	nuovoCanale->size = size;
	nuovoCanale->n_free=size;
	nuovoCanale->first_free=0;
	nuovoCanale->first_unread=0;

	return indice;	
}

extern "C" void c_channel_receive(natl chan_id){
	if(chan_id>=next_channel){
		flog(LOG_WARN, "Nella chiamata a receive, il canale inserito non esiste");
		c_abort_p();
		return;
	}

	des_channel* canale = &(array_deschan[chan_id]);

	if(canale->n_free==canale->size){
		inserimento_lista(canale->wait_r,esecuzione);
		schedulatore();
		return;
	}

	esecuzione->contesto[I_RAX] = canale->msg_buf[canale->first_unread];
	canale->first_unread = (canale->first_unread+1) % canale->size;
	canale->n_free++;

	if(!canale->wait_w) return;

	des_proc* nuovoProcesso = rimozione_lista(canale->wait_w);
	canale->msg_buf[canale->first_free] = nuovoProcesso->msg;
	canale->first_free = (canale->first_free + 1) % canale->size;
	canale->n_free--;
	inspronti();
	inserimento_lista(pronti, nuovoProcesso);
	schedulatore();
	return;
