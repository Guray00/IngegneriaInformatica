...
extern "C" void c_writepipe(natl p, char* buf, natl n){
	if(p>=nextpipe || buf==nullptr|| n<=0){
		flog(LOG_ERR, "errore negli input di writepipe");
		c_abort_p();
		return;
	}
	des_pipe* pipe = &array_despipe[p];
	if(!pipe->r_wait){
		pipe->w_wait = esecuzione;
		pipe->w_buf = buf;
		pipe->w_pending = n;
		schedulatore();
		return;
	...
	if(n < pipe->r_pending){
		for(natl i=0; i<n; i++) pipe->r_buf[i] = buf[i];
		pipe->r_buf+=n;
		pipe->r_pending-=n;
		return;
	}
	if(n > pipe->r_pending){
		for(natl i=0; i<pipe->r_pending; i++) pipe->r_buf[i] = buf[i];
		pipe->w_wait = esecuzione;
		pipe->w_buf = buf+pipe->r_pending;
		pipe->w_pending = n-pipe->r_pending;
		inserimento_lista(pronti,pipe->r_wait);
		pipe->r_wait=nullptr;
		schedulatore();
		return;
	}
...
extern "C" void c_readpipe (natl p, char* buf, natl n){
	if(p>=nextpipe || buf==nullptr|| n<=0){
		flog(LOG_ERR, "errore negli input di readpipe");
		c_abort_p();
		return;
	}
	des_pipe* pipe = &array_despipe[p];
	if(!pipe->w_wait){
		pipe->r_wait = esecuzione;
		pipe->r_buf = buf;
		pipe->r_pending = n;
		schedulatore();
		return;
	...
	if(n < pipe->w_pending){
		for(natl i=0; i<n; i++) buf[i] = pipe->w_buf[i];
		pipe->w_buf+=n;
		pipe->w_pending-=n;
		return;
	}
	if(n > pipe->w_pending){
		for(natl i=0; i<pipe->w_pending; i++) buf[i] = pipe->w_buf[i];
		pipe->r_wait = esecuzione;
		pipe->r_buf = buf+pipe->w_pending;
		pipe->r_pending = n-pipe->w_pending;
		inserimento_lista(pronti,pipe->w_wait);
		pipe->w_wait=nullptr;
		schedulatore();
		return;
	}
