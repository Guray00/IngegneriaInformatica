...
class getPaddr{
	...
	public:
		...
		paddr operator()(vaddr address){
			natl prev = nFrame;
			nFrame = vdf[nFrame].next_shmem;
			return prev*DIM_PAGINA;
		}
};
...
extern "C" void c_shmem_attach(natl id){
	...
	des_shmem* work = &array_desshmem[id];
	natl dim = work->npag*DIM_PAGINA;
	if(esecuzione->avail_addr+dim>fin_utn_c){
		esecuzione->contesto[I_RAX] = 0;
		return;
	}
	if(esecuzione->avail_addr+dim < esecuzione->avail_addr){
		esecuzione->contesto[I_RAX] = 0;
		return;
	}

	getPaddr m(work->first_frame_number);

	
	vaddr end = map(	
			esecuzione->cr3,
			esecuzione->avail_addr,
			esecuzione->avail_addr+dim,
			BIT_RW | BIT_US,		
			m
	);

	if(end != esecuzione->avail_addr+dim){
		unmap(	esecuzione->cr3,
				esecuzione->avail_addr,
				end,
				[](vaddr p, int) {});
		esecuzione->contesto[I_RAX] = 0;
