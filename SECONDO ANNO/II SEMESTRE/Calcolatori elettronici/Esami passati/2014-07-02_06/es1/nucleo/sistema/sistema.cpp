...
struct barriera{
	des_proc* listaAttesa;
	natl quantiRegistrati;
	natl quantiArrivati;
};

barriera bar = {nullptr, 0, 0};

void apriBarriera(){
	inspronti();
	for(natl i=0; i<bar.quantiArrivati; i++){
		des_proc * tmp = rimozione_lista(bar.listaAttesa);
		inserimento_lista(pronti,tmp);
	}
	schedulatore();
	bar.quantiArrivati=0;
}
...
extern "C" void c_dereg(){
	...
	esecuzione->registrato=false;
	bar.quantiRegistrati--;
	if(bar.quantiRegistrati==bar.quantiArrivati && bar.quantiRegistrati!=0){
		apriBarriera();
	}
	return;
}
...
extern "C" void c_barrier(){
	...
	inserimento_lista(bar.listaAttesa,esecuzione);
	bar.quantiArrivati++;
	schedulatore();
	if(bar.quantiRegistrati==bar.quantiArrivati ){
		apriBarriera();
	}
}
