...
// ( ESAME 2011-07-06
	if (esecuzione->interrupted) {
		...
		esecuzione->contesto[I_RAX] = n;
		return;
	}
	esecuzione->sleeping = true;
//   ESAME 2011-07-06 )
	richiesta *p;
...
// ( ESAME 2011-07-06
extern "C" bool c_interrupted()
{
	bool interrupted = esecuzione->interrupted;
	esecuzione->interrupted = false;
	return interrupted;
}
//  ESAME 2011-07-06 )
...
extern "C" void c_interrupt(natl id){
	...
	richiesta* q = p;
	natl contatore = p->d_attesa;

	while(p->pp != dest){
		q = p;
		p = p->p_rich;
		contatore+= p->d_attesa;
	}

	if(p->p_rich) p->p_rich->d_attesa += p->d_attesa;
