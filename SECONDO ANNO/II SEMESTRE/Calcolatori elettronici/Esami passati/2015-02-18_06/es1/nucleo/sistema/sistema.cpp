...
extern "C" void c_connect(natl id, natl dest)
{
	...
	des_proc *work = rimozione_lista(as->waiting);
	if (!as->waiting)
		as->state = SOCK_AVAIL;
...
extern "C" void c_close(natl id){
	...
	if(s->state==SOCK_CLOSING || s->state==SOCK_AVAIL){
		esecuzione->contesto[I_RAX] = (natq)false;
		return;
	}
	...
	if(s->state==SOCK_ACCEPTING){
		inspronti();
		while(s->waiting){
			des_proc* tmp = rimozione_lista(s->waiting);
			tmp->contesto[I_RAX] = 0xFFFFFFFF;
			inserimento_lista(pronti, tmp);
		}
		s->state=SOCK_AVAIL;
		schedulatore();
		return;
	...
	if(s->peer->state==SOCK_CLOSING){
		...
		while(s->peer->waiting){
			des_proc* tmp = rimozione_lista(s->peer->waiting);
			inserimento_lista(pronti, tmp);
		}
		s->peer->peer = nullptr;
		s->peer->state = SOCK_AVAIL;
		s->peer = nullptr;
		s->state = SOCK_AVAIL;
		...
		for(des_proc* tmp = s->waiting; tmp; tmp = tmp ->puntatore)
			tmp->contesto[I_RAX] = 0xFFFFFFFF;
