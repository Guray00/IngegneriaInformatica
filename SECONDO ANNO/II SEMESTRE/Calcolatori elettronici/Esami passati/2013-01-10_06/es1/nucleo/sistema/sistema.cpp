...
// ( SOLUZIONE 2013-01-10 #2
	...
	if(p->parent!=nullptr){
		...
		if(p->parent->waiting){
			inserimento_lista(pronti, p->parent->waiting);
			p->parent->waiting = nullptr;
		}
...
extern "C" void c_join() {
	if (esecuzione->own->nchildren == 0)
		return;
	inserimento_lista(esecuzione->own->waiting, esecuzione);
	schedulatore();
}
//   ESAME 2013-01-10 )
